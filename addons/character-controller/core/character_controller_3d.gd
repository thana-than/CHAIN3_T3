...
@onready var gravity_ability: GravityAbility3D = get_node(NodePath("Gravity Ability 3D"))
...
func move(_delta: float, input_axis := Vector2.ZERO, input_jump := false, input_crouch := false, input_sprint := false, input_swim_down := false, input_swim_up := false) -> void:
	...
	gravity_ability.set_active(input_jump and not is_on_floor() and not is_fly_mode() and not swim_ability.is_floating() and not swim_ability.is_submerged())
...
func _start_variables():
	...
	gravity_ability.gravity_multiplier = jump_gravity_multiplier
